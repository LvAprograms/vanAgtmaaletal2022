...
#ifdef H5_1_8
	#pragma message "Using HDF5 1.8 function calls."
#elif H5_1_6
	#pragma message "Using HDF5 1.6 function calls."
#else
    #error \
"Either H5_1_6 or H5_1_8 compile time defines has to be defined \
during compilation! Use -DH5_1_8 or -DH5_1_6 compiler flags \
according to HDF5 library version."
#endif


void hdf5_error(char const*const msg, char const*const file,
                int const line, ...)
{
    va_list args;
    va_start(args, msg);
    ...
    // note: asprintf is only available on posix not on windows
    int const ret = asprintf(&msg_, "ERROR in %s line %d: %s!\n", file,
                             line, msg);
    
    if (ret < 0) {
        fprintf(fp_log, "ERROR: asprintf failed!\n");
        fflush(fp_log);
        exit(1);
    }
    
    vfprintf(fp_log, msg_, args);
    va_end(args);
    
    fprintf(fp_log, "Printing HDF5 error stack...\n");
    
    #if H5_1_8
        H5Eprint(H5E_DEFAULT, fp_log);
    #endif
...
void create_output_hdf5( const char name[] )
{
    ...
    if (file_id < 0) {
        hdf5_error("failed to create HDF5 file '%s'",
    ...
    if (file_id < 0) {
        hdf5_error("failed to close file identifier, for file '%s'",
                   __FILE__, __LINE__, name);
    }
}
...
{
	hid_t       file_id = 0, group_id = 0;  /* identifiers */
	herr_t      status = 0;
	char        *group_name;
    ...
    if (file_id < 0) {
        hdf5_error("failed to open HDF5 file '%s'", __FILE__, __LINE__,
                    filename);
    }
...
#ifdef H5_1_8
	group_id = H5Gcreate(file_id, group_name,
                         H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
#elif H5_1_6
	/* group_id = H5Gcreate(file_id, group_name, H5P_DEFAULT); */
	/* Use default HDF5 heap size by passing -1 */ 
    group_id = H5Gcreate(file_id, group_name, -1);
#endif

    if (group_id < 0) {
        hdf5_error("failed to create group id for group '%s' in "
                   "file '%s'", __FILE__, __LINE__, group, filename);
    }
    ...
    if (status < 0) {
        hdf5_error("failed to close group id while creating group '%s' "
                   "in file '%s'", __FILE__, __LINE__, group, filename);
    }
	
	/* Close the file. */
	status = H5Fclose(file_id);
    if (status < 0) {
        hdf5_error("failed to close file id while creating group '%s' "
                   "in file '%s'", __FILE__, __LINE__, group, filename);
    }
							 ...
							 int np, void* data, int dim )
{
    ...
    if (file_id < 0) {
        hdf5_error("failed to open file '%s'", __FILE__, __LINE__,
                    filename);
    }
    ...
    if (particle_group_id < 0) {
        hdf5_error("failed to create particle group '%s' in '%s'",
                   __FILE__, __LINE__, group_name, filename);
    }
    ...
    if (plist == -1) {
        hdf5_error("failed to create property list in '%s'",
    ...
    if (status < 0) {
        hdf5_error("failed to set chunk while saving data to file '%s'",
                    __FILE__, __LINE__, filename);
    }
    ...
    if (status < 0) {
        hdf5_error("failed to set deflation level while "
                   "saving data to file '%s'", __FILE__, __LINE__,
                    filename);
    }
    ...
    if (coord_dataspace_id < 0) {
        hdf5_error("failed to create data space for dataset while "
                   "saving data to file '%s'", __FILE__, __LINE__,
                    filename);
    }
    ...
    if (attr_dataspace_id < 0) {
        hdf5_error("failed to create attribute for particle field in "
                   "file '%s'", __FILE__, __LINE__, filename);
    }
    ...
    if (attribute_id < 0) {
        hdf5_error("failed to create length attribute for particle field in "
                   "file '%s'", __FILE__, __LINE__, filename);
    }
    ...
    if (status < 0) {
        hdf5_error("failed to write particle data dimensions to "
                   "file '%s'", __FILE__, __LINE__, filename);
    }
    ...
    if (status < 0) {
        hdf5_error("failed to close dimension attribute identifier "
                   "while writing to file '%s'", __FILE__, __LINE__,
                    filename);
    }
    ...
    if (status < 0) {
        hdf5_error("failed to close attribute dataspace identifier "
                   "while writing to file '%s'", __FILE__, __LINE__,
                    filename);
    }
    ...
    if (status < 0) {
        hdf5_error("failed to close property list identifier "
                   "while writing to file '%s'", __FILE__, __LINE__,
                    filename);
    }
    ...
    if (status < 0) {
        hdf5_error("failed to close coordinate dataspace identifier "
                   "while writing to file '%s'", __FILE__, __LINE__,
                    filename);
    }
    ...
    if (status < 0) {
        hdf5_error("failed to close coordinate dataset identifier for "
                   "dataset '%s' while writing to file '%s'",
                   __FILE__, __LINE__, dataset_name, filename);
    }
    ...
    if (status < 0) {
        hdf5_error("failed to close particle group identifier "
                   "while writing to file '%s'", __FILE__, __LINE__,
                    filename);
    }
    ...
    if (status < 0) {
        hdf5_error("failed to close HDF5 file '%s'",
                    __FILE__, __LINE__, filename);
    }
...
int create_hdf5( int compress, char* txtout, int n0 )
{
    ...
    if (compo_arr == NULL) {
		fprintf(fp_log,"ERROR: failed to allocate memory for "
                       "composition array!\n");
		fflush(fp_log);
		exit(1);
    }
...
int create_hdf5_markerprop( int compress, char* txtout, int n0 )
{
    ...
    int ret = 0;
	if(compress==_TRUE_) {
		ret = asprintf( &name_me, "%s%s",txtout,".gzip.h5");
	}
	else {
		ret = asprintf( &name_me, "%s%s" ,txtout, ".h5" );
	}
	
    
    if (ret < 0) {
		fprintf(fp_log,"ERROR: failed to allocate memory for "
                       "hdf5 filename string!\n");
		fflush(fp_log);
		exit(1);
    }
