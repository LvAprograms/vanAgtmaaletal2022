...
int main()
{
#ifdef STM_LOG_MARKER
    FILE*const fp_marker = fopen("PTt.log", "w");

    if (fp_marker == NULL) {
        fprintf(stderr, "failed to open PTt.log\n");
        exit(1);
    }

    struct Rectangle rect = {
        .is_set = Set,
        .xmin = 900.0 * 1000, .xmax = 1500.0 * 1000,
        .ymin = 22.0 * 1000, .ymax = 350.0 * 1000,
    };

    struct MarkerLogger logger = {
        .is_found = NotFound,
        .rock_id = 5,
        .rect = rect,
    };
	...
	for (f0=fln3;f0<fl0num;f0++)
	{
		...
		for (n0=0;n0<cyc0max;n0++)
		{
			...
			for (int i = 0; i < 5; i++) {
				int const index = search_for_marker(&logger, i);
				if (index != -1) {
					fprintf(fp_marker, "%d %d %lf %lf %lf %lf %lf\n",
							index, markt[index], timesum/3.15576e+7, markx[index], marky[index],
							markp[index], markk[index]);
				}
			}
				fflush(fp_marker);
